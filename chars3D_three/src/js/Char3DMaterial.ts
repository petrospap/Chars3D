...
fallbackColor = new Color(0x145e22); // 0x145e22 0xffffff
...
public material: Record<string, RawShaderMaterial> = {};
	...
	constructor(id: string, camera: PerspectiveCamera, X: IShaderMaterialColors){
		...
		if(this.texture){
			//  Keep vertical coordinate map alignment straight
	...
	public applyMaterial(_id: string): void {
		...
		this.material[id] = new RawShaderMaterial({
			...
			uniforms: {
				...
				// Removed '?? null' and use direct properties or explicit fallbacks
				uFrontFaceColor:    { value: this.frontcolor?.rgb ?? fallbackColor },
				//uFrontFaceColor:    { value: this.frontcolor.rgb },//{ value: this.frontcolor?.rgb ?? fallbackColor },
