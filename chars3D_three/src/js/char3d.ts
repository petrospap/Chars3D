...
import {_SETTINGS, Color3, Color4} from './settings';
import {Char3DMaterial} from './Char3DMaterial';
import {AtlasAssemble} from './AtlasAssemble';
import {Atlas} from './AtlasFactory';
import type {IColor4, IPos, Ixy, IChars, IDrawOptions, IParagraphState, IUpdateData, IUpdateButton, IEnableOrDisable, IBoundingBox, IBoundingInfo, IG3D, IBlitBuffers} from './interfaces';
/**
...
*/
	...
	private static setData(X: IDrawOptions): IParagraphState {
		...
		//console.log('id: '+X.id+' _frontcolor? ',_frontcolor);

		if(X.font3d && info.is3d || X.font3d === 0){
			_font3D = X.font3d,
...
*/
	private static setParameters(update: boolean, P: IParagraphState): void {
		...
		for (; i < Len; i++) {
			...
			//if(P.txt[i] === 'W'){
			//	console.log('font: '+P.font+' letter: W g: ',g)
			//}
