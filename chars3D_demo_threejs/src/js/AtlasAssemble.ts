...
import type {IGlyphBuffers, IBackgroundGeometry} from './interfaces';
...
// CRITICAL VERIFICATION: BORDER MATH RE-ALIGNMENT
// If border is true, we have TWO loops of points (Inner + Outer) = POINTERS_PER_RING * 2
...
// AtlasAssemble: v 0.0.3 Three final
...
private _vertexPanelBuffer: ArrayBuffer;
    ...
    public addSource(letter: string, g: IGlyphBuffers): void {
        ...
        if (_source) {
			// If this letter has already compiled, get only the lengths
	...
	private allocate(): void {
		...
		// The absolute minimum bytes required for this text update pass
	...
	public reallocate(background: boolean, border: boolean, sizeMightChange: boolean): void {
		...
		// growth check triggers only if unique characters break boundaries!
	...
	public assemble2D(key: string, x: number, y: number, z: number, size: number): void {
		...
		this.pOffset += pLen; //source.L;
		this.iOffset += iLen;
		this.vBase += (pLen / 3) //(source.L / 3)
	};
...
/**
 * Assemble a unified rounded-corner background panel with an optional border stroke.
...
*/
	public assemblePanel(cx: number, cy: number, cz: number, width: number, height: number, radius: number, thickness: number, drawBackground: boolean, drawBorder: boolean): void {
		if (drawBackground || drawBorder) {
			// 1. REBUILD THE ISOLATED PANEL GEOMETRY IF SIZE HAS CHANGED
			if (this.sizeMightChange) {
				...
				// BLIT BACKGROUND TO ISOLATED STORAGE
				if (drawBackground) {
					const bgBaseVtx = 0;
					const vIdx3 = vIdx * 3;
					this.PP[vIdx3] = cx; 
				...
				// BLIT BORDER TO ISOLATED STORAGE
				if (drawBorder) {
			 ...
			 * Update pointers to include both the text and panel elements,
			 * specifying the full size of the sub-array slice for Babylon.js.
			 */
	...
	public assembleButton(key: string, x: number, y: number, z: number, size: number, radius: number, thickness: number, padding:number[], hasBg: boolean, hasborder: boolean, width: number, height: number, worldCX: number, worldCY: number): void {
		...
		const pAttr = new BufferAttribute(this.P.slice(0, this.pOffset), 3);
		pAttr.name = key+'_position';
		btnG.setAttribute('position', pAttr);
		
		// NORMALS
		const nAttr = new BufferAttribute(this.N.slice(0, this.pOffset), 3);
		nAttr.name = key+'_normal';
		btnG.setAttribute('normal', nAttr);
		
		// UV COORDINATES
		const uAttr = new BufferAttribute(this.U.slice(0, this.vBase * 2), 2);
		uAttr.name = key+'_uv';
		btnG.setAttribute('uv', uAttr);
		
		// FACE IDS SHADER
		const fidAttr = new BufferAttribute(this.FID.slice(0, this.vBase), 1);
		fidAttr.name = key+'_aFaceId';
		btnG.setAttribute('aFaceId', fidAttr);
		
		// INDICES
		const iAttr = new BufferAttribute(this.I.slice(0, this.iOffset), 1);
		iAttr.name = key+'_indice';
		btnG.setIndex(iAttr);
		return btnG

	};

	public createVDS(): void {
		// POSITIONS 
		this.pAttr = new BufferAttribute(this.P, 3);
		this.pAttr.setUsage(DynamicDrawUsage); 
		this.vds.setAttribute('position', this.pAttr);
		
		// NORMALS
		this.nAttr = new BufferAttribute(this.N, 3);
	    this.nAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('normal', this.nAttr);
		
		// UV COORDINATES
		this.uAttr = new BufferAttribute(this.U, 2);
		this.uAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('uv', this.uAttr);
		
		// FACE IDS SHADER
		this.fidAttr = new BufferAttribute(this.FID, 1);
		this.fidAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('aFaceId', this.fidAttr);
		
		// INDICES
		this.iAttr = new BufferAttribute(this.I, 1);
		this.iAttr.setUsage(DynamicDrawUsage);
		this.vds.setIndex(this.iAttr)

	};

	public updateVDS(): void {

		// POSITIONS 
		const posAttr = new BufferAttribute(this.P.subarray(0, this.pOffset), 3);
		posAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('position', posAttr);

		// NORMALS
		const normAttr = new BufferAttribute(this.N.subarray(0, this.pOffset), 3);
		normAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('normal', normAttr);

		// UV COORDINATES
		const uvAttr = new BufferAttribute(this.U.subarray(0, this.vBase * 2), 2);
		uvAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('uv', uvAttr);

		// FACE IDS SHADER
		const fidAttr = new BufferAttribute(this.FID.subarray(0, this.vBase), 1);
		fidAttr.setUsage(DynamicDrawUsage);
		this.vds.setAttribute('aFaceId', fidAttr);

		// INDICES
		const indexAttr = new BufferAttribute(this.I.subarray(0, this.iOffset), 1);
		indexAttr.setUsage(DynamicDrawUsage);
		this.vds.setIndex(indexAttr);

		// TREEJS UPDATE CONSTRAINT
		this.vds.setDrawRange(0, this.iOffset)
	};
	...
	public getAssembledStats(): IStats {
		...
		// Total Vertices/Polygons (triangle) from offset's
		let textVertices = this.pOffset / 3;
		let textPolygons = this.iOffset / 3; 

		// get the absolute physical byte size from text master buffers
		let tb = 0; 
		tb += this.P.byteLength;
		tb += this.N.byteLength;
		tb += this.U.byteLength;
		tb += this.FID.byteLength;
		tb += this.I.byteLength;
		
		/* background/border panel cache check */
		if (this.PP && this.PP.byteLength) {
			
			tb = tb * 2; // doubles memory, as background/border has the same memory copied from text
			panelVertices = this.panelP / 3;
			panelPolygons = this.panelI / 3;
			
			// Isolate the pure Vertices/Polygons from the panelP/panelI slabs
	...
	public clean(): void {
		// nullify the main TypedArray buffer
		this.P = null!;
		this.N = null!;
		this.U = null!;
		this.FID = null!;
