...
import type {Ixy, IG3D, IFontData, IGlyphXBounds, IGlyphBuffers, IBlitBuffers} from './interfaces.ts';
// v0.9.7 bevel working, final clean, minify + NO MAP anymore! plain JSON
...
setGBounds = (bounds: number[]): IGlyphXBounds => {
    ...
    const minX = !xMin ? 0.5 : xMin * _SIZE;
    const minY = !yMin ? 0.5 : yMin * _SIZE;
    const maxX = !xMax ? 0.5 : xMax * _SIZE;
    const maxY = !yMax ? 0.5 : yMax * _SIZE;
...
/* optional count total letters and bytes */
...
static MITER_LIMIT: number = 0.8; // Cap the spike miter of the bevel width
	...
	constructor(GLYPHS: IG3D, BLIT: IBlitBuffers, hasblit: boolean, hasfont: boolean) {
		...
		Atlas.cleanFonts()
		//console.log('Atlas fonts',Atlas.fonts); 
		...
		for (const [name, font] of Object.entries(Atlas.fonts)) {
			...
			for (const [letter, g] of Object.entries(font)) {
				if (g._v) {
					...
					if(g._v != 1){
						// MANDATORY to replace [] to 1. is a FLAG that we have glyphs. 
						g._v = 1
					};

					if(g._x){
						// remove opentype bounds
						delete g._x
					};
					...
					if(is3d){
						...
						if(_SETTINGS.debugTime){
							Atlas.letters2D++;
							Atlas.totalbytesd += g.P.byteLength;
	...
	* @param _b?:             	// back/front info
		* @param f: number;		// front vertex count
		* @param v: number[];	// vertices2D coords
	* optional pass
	* @param ascender: number,
	* @param descender: number
	*/
	...
	private static build3D(g: IG3D): void {
		...
		for (const poly of g._o) {
			...
			paths.forEach(({ pts, hole }) => {
				...
				for (let i = 0; i < count; i++) {
					...
					//if (miterLen > _SETTINGS.MITER_LIMIT) miterLen = _SETTINGS.MITER_LIMIT;
		...
		*/
		...
		for (; bi < _FC; bi++) {
			...
			// Scale them uniformly to match your 3D world sizing boundaries
			const scaledX = rawX * _SIZE;
			const scaledY = rawY * _SIZE;

			// Push back cap geometry to buffer at maximum depth position
			// (t = 1.0 or depth depending on your layout arrangement)
		...
		*/
		...
		/* Store Raw Indices for independent button meshes if needed */
